...
JITCompiler::JITCompiler() : config_(detectDefaultConfig()) {}

JITCompiler::JITCompiler(const CompilerConfig& config) : config_(config) {}
...
std::string JITCompiler::createTempDirectory() {
    ...
    for (int attempt = 0; attempt < 10; ++attempt) {
        std::string dir_name = "clangjit_" + std::to_string(pid) + "_" + std::to_string(dis(gen));
