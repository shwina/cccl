...
CompilerConfig detectDefaultConfig() {
    ...
    // Detect clangjit include path
    if (const char* env = std::getenv("CLANGJIT_INCLUDE_PATH")) {
        config.clangjit_include_path = env;
    }
#ifdef CLANGJIT_INCLUDE_DIR
    else {
        config.clangjit_include_path = CLANGJIT_INCLUDE_DIR;
    }
#endif
...
bool validateConfig(const CompilerConfig& config, std::string* error_message) {
    ...
    if (config.sm_version < 30 || config.sm_version > 150) {
        if (error_message) {
            *error_message = "Invalid SM version: " + std::to_string(config.sm_version) +
    ...
    if (config.optimization_level < 0 || config.optimization_level > 3) {
        if (error_message) {
            *error_message = "Invalid optimization level: " + std::to_string(config.optimization_level) +
