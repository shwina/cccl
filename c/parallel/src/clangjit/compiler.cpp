...
public:
    ...
    std::string& diagnostics)
  {
    ...
    arg_strings.push_back(config.clangjit_include_path + "/clangjit/cuda_minimal/stubs");
    arg_strings.push_back("-internal-isystem");
    ...
    arg_strings.push_back(config.clangjit_include_path + "/clangjit/cuda_minimal/__clang_cuda_runtime_wrapper.h");
    ...
    for (int attempt = 0; attempt < 2; ++attempt)
    {
      ...
      if (!device_pch_path.empty() && std::filesystem::exists(device_pch_path))
      {
        ...
        if (config.verbose)
        {
          diagnostics += "Using device PCH: " + device_pch_path + "\n";
        }
      }
      ...
      success = compiler.ExecuteAction(emit_llvm_action);
      ...
      // Detect stale PCH on first attempt and retry once with a fresh PCH.
      if (!success && attempt == 0 && !device_pch_path.empty()
          && diag_output.find("has been modified since the precompiled header") != std::string::npos)
      {
        std::filesystem::remove(device_pch_path);
        auto pch_src_path = getPCHSourcePath("device", config.sm_version);
        std::string pch_regen_diag;
        if (generatePCH(pch_preamble_source, pch_src_path, device_pch_path, arg_strings, pch_regen_diag))
        {
          if (config.verbose)
          {
            diagnostics += "Stale PCH detected and regenerated: " + device_pch_path + "\n";
          }
          continue; // retry compilation with the fresh PCH
        }
        diagnostics += "PCH regeneration after stale detection failed: " + pch_regen_diag + "\n";
      }
  ...
  BitcodeResult compileToDeviceBitcode(const std::string& source_code, const CompilerConfig& config)
  {
    ...
    std::string temp_dir =
      (std::filesystem::temp_directory_path() / ("clangjit_bc_" + std::to_string(reinterpret_cast<uintptr_t>(this))))
        .string();
    ...
    std::string& diagnostics)
  {
    ...
    for (int attempt = 0; attempt < 2; ++attempt)
    {
      ...
      if (!host_pch_path.empty() && std::filesystem::exists(host_pch_path))
      {
        ...
        if (config.verbose)
        {
          diagnostics += "Using host PCH: " + host_pch_path + "\n";
        }
      }
      ...
      success = compiler.ExecuteAction(emit_action);
      ...
      // Detect stale PCH on first attempt and retry once with a fresh PCH.
      if (!success && attempt == 0 && !host_pch_path.empty()
          && diag_output.find("has been modified since the precompiled header") != std::string::npos)
      {
        std::filesystem::remove(host_pch_path);
        auto pch_src_path = getPCHSourcePath("host", config.sm_version);
        std::string pch_regen_diag;
        if (generatePCH(pch_preamble_source, pch_src_path, host_pch_path, arg_strings, pch_regen_diag))
        {
          if (config.verbose)
          {
            diagnostics += "Stale host PCH detected and regenerated: " + host_pch_path + "\n";
          }
          continue; // retry compilation with the fresh PCH
        }
        diagnostics += "Host PCH regeneration after stale detection failed: " + pch_regen_diag + "\n";
      }
  ...
  compileToObject(const std::string& source_code, const std::string& output_path, const CompilerConfig& config)
  {
    ...
    std::string temp_dir =
      (std::filesystem::temp_directory_path() / ("clangjit_" + std::to_string(reinterpret_cast<uintptr_t>(this))))
        .string();
    std::filesystem::create_directories(temp_dir);
