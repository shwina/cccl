...
static_assert(device_merge_sort_policy()(::cuda::arch_id{{CUB_PTX_ARCH / 10}}) == {10}, "Host generated and JIT compiled policy mismatch");
  ...
  static_assert(std::is_trivially_copyable_v<cub::detail::merge_sort::policy_selector>);
  build_ptr->runtime_policy      = std::malloc(sizeof(cub::detail::merge_sort::policy_selector));
  build_ptr->runtime_policy_size = sizeof(cub::detail::merge_sort::policy_selector);
  std::memcpy(build_ptr->runtime_policy, &policy_sel, sizeof(cub::detail::merge_sort::policy_selector));
