...
int main() {
  // Define CUDA code that uses CUB DeviceReduce::Reduce with gpu_to_gpu determinism
...
extern "C" CLANGJIT_EXPORT int computeSumDeterministic(float* d_input, int num_items, float* result) {
  ...
  std::cout << "ClangJIT Example: CUB DeviceReduce with gpu_to_gpu determinism\n\n";
  ...
  std::cout << "Compiling CUDA code with CUB (deterministic reduction)...\n";
  ...
  auto computeSumDeterministic =
      compiler.getFunction<int (*)(float *, int, float *)>("computeSumDeterministic");
  ...
  for (int i = 0; i < num_runs; ++i) {
    ...
    if (run_result != result) {
      std::cerr << "Run " << i << " produced different result: " << run_result
                << " vs " << result << "\n";
      deterministic = false;
    }
  }
  ...
  if (success) {
    std::cout << "Results verified successfully! (" << num_runs
