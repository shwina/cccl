...
from ._caching import CachableFunction, cache_with_keyer
...
# _WellKnownOp: extract (kind.name, kind.value)
cache_with_keyer.register(_WellKnownOp, lambda op: (op._kind.name, op._kind.value))

# _StatelessOp: extract CachableFunction
cache_with_keyer.register(_StatelessOp, lambda op: op._cachable)

# Note: OpKind enums and raw Callables are handled by make_op_adapter()
# before caching, so they don't need registry entries. The pattern is:
#   1. Public API accepts Callable | OpKind
#   2. Normalize with make_op_adapter()
#   3. Call _make_*_cached() with OpAdapter for consistent caching
