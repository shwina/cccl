...
def _infer_input_type_from_annotations(op_adapter) -> TypeDescriptor | None:
    """Infer input type from a function's annotations (for output iterators).

    Returns the first parameter's type annotation if available, None otherwise.
    """
    import inspect

    # For CompiledOp, we can't infer
    if isinstance(op_adapter, CompiledOp):
        return None

    # For JIT ops, check annotations
    if hasattr(op_adapter, "func") and op_adapter.func is not None:
        from .._jit import get_type_descriptor
        from ..numba_utils import as_numba_type

        sig = inspect.signature(op_adapter.func)
        params = list(sig.parameters.values())
        if params and params[0].annotation != inspect.Parameter.empty:
            try:
                numba_type = as_numba_type(params[0].annotation)
                return get_type_descriptor(numba_type)
            except Exception:
                pass
...
def TransformOutputIterator(
    ...
    # Try to infer the input type from the function's annotations.
    input_type = _infer_input_type_from_annotations(op_adapter)

    if input_type is None:
        # Fallback: use underlying type (may not always be correct)
        input_type = _get_value_type(underlying)

    return _TransformIterator(
        underlying=underlying_iter,
        transform_op=op_adapter,
        output_value_type=input_type,  # This is the type algorithm writes
        is_input=False,
    )
