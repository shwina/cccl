...
def make_struct_type(name, field_names, field_types):
    ...
    class StructType(numba.types.Type):
        ...
        def can_convert_from(self, typingctx, other):
            """Allow implicit conversion from tuples to this struct."""
            if isinstance(other, (types.Tuple, types.UniTuple)):
                # Check tuple has correct number of elements
                if isinstance(other, types.UniTuple):
                    if other.count == len(field_spec):
                        return numba.core.typeconv.Conversion.safe
                else:
                    if len(other.types) == len(field_spec):
                        return numba.core.typeconv.Conversion.safe
            return None
    ...
    # Register implicit casting from tuple to struct
    # This allows Numba to automatically convert tuples to structs when needed
    num_fields = len(field_spec)
    ...
    def cast_tuple_to_struct(context, builder, fromty, toty, val):
        ...
        if isinstance(fromty, types.UniTuple):
            if fromty.count != num_fields:
                return None
            tuple_types = [fromty.dtype] * fromty.count
        else:
            if len(fromty.types) != num_fields:
                return None
