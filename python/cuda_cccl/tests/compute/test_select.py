...
import cuda.compute
from cuda.compute import CacheModifiedInputIterator, StatefulOp, ZipIterator, gpu_struct
...
def test_select_with_zip_iterator(monkeypatch):
    ...
    # Disable SASS check since DiscardIterator with ZipIterator may produce LDL/STL
    monkeypatch.setattr(
...
def test_select_stateful():
    ...
    threshold_value = 50
    state = cp.array([threshold_value], dtype=np.int32)

    # Define stateful condition: uses state to track a threshold
    # State is passed as a pointer (CPointer to global memory)
    def threshold_select(state_ptr, x):
        # state_ptr[0] contains the threshold value
        threshold = state_ptr[0]
        return x > threshold

    # Wrap in StatefulOp
    op = StatefulOp(state, threshold_select)
    ...
    # Verify state is still accessible and unchanged
    assert int(state[0].get()) == threshold_value
...
def test_select_stateful_atomic():
    ...
    state = cp.zeros(1, dtype=np.int32)

    # Define stateful condition that counts rejections using atomics
    def count_rejects(state_ptr, x):
        if x > 50:
            return True
        else:
            # Use custom atomic add intrinsic that works directly on pointers
            atomic_add_ptr(state_ptr, 0, 1)
            return False

    # Wrap in StatefulOp
    op = StatefulOp(state, count_rejects)
    ...
    rejected_count = int(state[0].get())
