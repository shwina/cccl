...
def test_compiled_iterator_counting():
    ...
    # New API: explicit state bytes + alignment
    counting_iter = CompiledIterator(
        state_bytes=np.int64(10).tobytes(),
        state_alignment=8,
...
def test_compiled_iterator_different_offset():
    ...
    # State: starting offset of 0 (explicit bytes + alignment)
    counting_iter = CompiledIterator(
        state_bytes=np.int64(0).tobytes(),
        state_alignment=8,
        value_type=types.int64,
        advance=advance_op,
        input_dereference=deref_op,
...
def test_compiled_iterator_validation_errors():
    ...
    with pytest.raises(TypeError, match="state_bytes must be bytes"):
        CompiledIterator(
            state_bytes="not bytes",
            state_alignment=8,
            value_type=types.int64,
            advance=advance_op,
            input_dereference=deref_op,
    ...
    with pytest.raises(
        ValueError, match="state_alignment must be a positive power of 2"
    ):
        CompiledIterator(
            state_bytes=np.int64(0).tobytes(),
            state_alignment=3,
...
def test_compiled_op_with_gpu_struct():
    ...
    # Define a structured dtype (numba-free)
    ...
    h_points = np.array([(1, 2), (3, 4), (5, 6), (7, 8)], dtype=point_dtype)
    d_points = cp.empty(len(h_points), dtype=point_dtype)
    d_points.set(h_points)

    d_out = cp.empty(1, dtype=point_dtype)
    h_init = np.zeros(1, dtype=point_dtype)
...
def test_compiled_iterator_properties():
    ...
    it = CompiledIterator(
        state_bytes=np.int64(42).tobytes(),
        state_alignment=8,
        value_type=types.int64,
        advance=advance_op,
        input_dereference=deref_op,
    ...
    assert memoryview(it.state).tobytes() == np.int64(42).tobytes()
    assert it.state_alignment == 8
